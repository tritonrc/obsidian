...
async fn test_services_endpoint_all_signals() {
    ...
    ingest_traces(&app, "worker", "process_job", &[1; 16], &[2; 8], 1_000_000_000, 2_000_000_000, 1).await;
    ...
    assert!(services[0]["signals"].as_array().unwrap().contains(&json!("metrics")));
    assert_eq!(services[1]["name"], "payments");
    assert!(services[1]["signals"].as_array().unwrap().contains(&json!("logs")));
    assert_eq!(services[2]["name"], "worker");
    assert!(services[2]["signals"].as_array().unwrap().contains(&json!("traces")));
}
