...
**Bad arguments are rejected, never ignored.** Before dispatch, `tools/call` validates every argument against the tool's own `inputSchema`: undeclared keys, values of the wrong declared type, and values outside a declared `enum` all return `isError:true` naming the key. Handlers read arguments with `as_str`/`as_u64` and treat anything else as absent, so each of these would otherwise be dropped in silence — and a dropped filter is invisible in the result, which still reports a `total_count` that reads exactly like a match (this cost real debugging time: 17,539 traces looked like a match for a filter key the tool never read). `integer` means what `as_u64` can read — a non-negative integer literal, since every integer argument here is a count or a checkpoint token — so `-5` and the formally-valid `50.0` are both refused rather than silently dropped; that bound is advertised as `minimum: 0`, and `status` advertises its three values, keeping the schema a client validates against in step with the server's verdict. Validating against the advertised schema rather than a hand-kept list keeps one source of truth, and every `inputSchema` sets `additionalProperties: false` so a client validating against `tools/list` reaches the same verdict as the server. The remaining gap is one-sided: a property added to a descriptor but never read by its handler would be accepted and ignored again — typed per-tool argument structs used for both deserialization and schema generation would close it.
