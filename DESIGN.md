...
**Error model.** Protocol-level failures (malformed JSON-RPC, unknown method, unknown/missing tool name, schema-invalid arguments) are JSON-RPC error objects (`-32600`/`-32601`/`-32602`/...). Semantically-bad-but-schema-valid input (unknown service, malformed query string, an out-of-range range query) returns a `tools/call` result with `isError:true` and a self-correcting message — injected back into the model so it fixes itself. Unknown-service errors echo the bad value and list the known services. **Undeclared argument keys are rejected, not ignored:** `tools/call` checks every argument key against the tool's own `inputSchema.properties` before dispatch and returns `isError:true` naming the key and listing the accepted ones. Silently dropping an unread key is the dangerous failure — the call still succeeds and its `total_count` reports the *unfiltered* total, which reads exactly like a match.
