...
**Error model.** The split follows the spec's two mechanisms. *Request structure* the model cannot fix is a JSON-RPC error object (`-32600`/`-32601`/`-32602`/...): malformed JSON-RPC, unknown method, unknown or missing tool name, and an `arguments` value that is not an object (absent and `null` are both accepted as "no arguments" — clients send either). *Input validation and semantics* return a `tools/call` result with `isError:true` and a self-correcting message, injected back into the model so it fixes itself: unknown service (echoing the bad value and listing the known ones), a malformed query string, an out-of-range range query.

**Bad arguments are rejected, never ignored.** Before dispatch, `tools/call` validates every argument against the tool's own `inputSchema`: undeclared keys, values of the wrong declared type, values outside a declared `enum`, and missing required arguments all return `isError:true` naming the key. An argument that reaches a handler unreadable is read as absent, so each of these would otherwise be dropped in silence — and a dropped filter is invisible in the result, which still reports a `total_count` that reads exactly like a match (this cost real debugging time: 17,539 traces looked like a match for a filter key the tool never read). `integer` means what a `u64` can hold — a non-negative integer literal, since every integer argument here is a count or a checkpoint token — so `-5` and the formally-valid `50.0` are both refused rather than silently dropped; that bound is advertised as `minimum: 0`, and closed value sets like `status`/`scope`/`detail` advertise their values, keeping the schema a client validates against in step with the server's verdict. Every `inputSchema` sets `additionalProperties: false`, so a client validating against `tools/list` reaches the same verdict as the server.

**One declaration per tool, so the schema and the handler cannot drift.** `src/mcp/tools/args/` declares each tool's arguments once as a typed struct (`tool_args!`), and that single field list produces all three consumers: the struct the handler reads, the `inputSchema` advertised by `tools/list`, and the allowlist the validator rejects against. Both drift directions are then mechanical failures rather than review items. A property cannot be advertised without a field behind it, because the schema is generated from the fields — there is nowhere to add one; a `tools/list` property no struct accepts fails the round-trip test in that module. And a field no handler reads is a `field is never read` build failure under CI's `-D warnings`, which is the direction that actually bit us: an argument accepted and then ignored is invisible in the response. A field's written type carries the rest — `Option<T>` is optional and anything else lands in `required`, `u64` advertises `minimum: 0`, and a `str_enum!` type advertises its own variants, so a handler matches on an enum instead of string-comparing and falling through to a default. Schema validation still runs ahead of deserialization purely for the message: serde reports `invalid type: integer 123, expected a string` without naming the field it was reading, and a message that does not name the argument cannot be acted on. Semantic checks stay in the handlers, where the knowledge lives — a well-typed `min_duration` can still be nonsense, `service` is only required by `reset`'s `service` scope, and only the store knows which services exist.
...
**Architecture.** `src/mcp/` holds `mod.rs` (router), `protocol.rs` (JSON-RPC envelope + version constants), `server.rs` (POST dispatch + transport compliance), `tools/` (`args/` typed arguments and the schema/validator machinery they generate, `descriptors.rs` for `tools/list`, `dispatch.rs` for `tools/call`, and one `handlers/` module per signal), and `synth/` (transport-free typed cores: `summarize_activity`, `check_health`, `describe_service`, `trace_item`, `build_trace_tree`). Handlers stay thin, and they no longer parse: dispatch hands each one its typed arguments, so a handler calls a core/evaluator and formats the response.
