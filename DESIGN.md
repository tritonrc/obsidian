...
**Error model.** The split follows the spec's two mechanisms. *Request structure* the model cannot fix is a JSON-RPC error object (`-32600`/`-32601`/`-32602`/...): malformed JSON-RPC, unknown method, unknown or missing tool name, and an `arguments` value that is not an object. One deliberate deviation: the request schema types `arguments` as an optional object, so a literal `null` is strictly malformed, but it is accepted as "no arguments" rather than refused — it is unambiguous, and a client that emits `null` for an absent optional field would otherwise be unable to call a no-argument tool at all. Absent and `null` therefore behave identically; every other non-object shape is a protocol error. *Input validation and semantics* return a `tools/call` result with `isError:true` and a self-correcting message, injected back into the model so it fixes itself: unknown service (echoing the bad value and listing the known ones), a malformed query string, an out-of-range range query.

**Bad arguments are rejected, never ignored.** Before dispatch, `tools/call` validates every argument against the tool's own `inputSchema`: undeclared keys, values of the wrong declared type, values outside a declared `enum`, and missing required arguments all return `isError:true` naming the key. An argument that reaches a handler unreadable is read as absent, so each of these would otherwise be dropped in silence — and a dropped filter is invisible in the result, which still reports a `total_count` that reads exactly like a match (this cost real debugging time: 17,539 traces looked like a match for a filter key the tool never read). `integer` means what a `u64` can hold — a non-negative integer literal, since every integer argument here is a count or a checkpoint token — so `-5` and the formally-valid `50.0` are both refused rather than silently dropped. The range is advertised as `minimum: 0` / `maximum: 18446744073709551615`, and closed value sets like `status`/`scope`/`detail` advertise their values, so a client validating against `tools/list` reaches the server's verdict in every case but one: JSON Schema counts `50.0` as an integer and the server does not, because `u64` deserialization cannot read it and accepting it would mean dropping it later — the exact failure this section exists to prevent. That one asymmetry is deliberate; the error message names the expectation ("must be a non-negative integer") so the caller can correct the spelling. Every `inputSchema` sets `additionalProperties: false`, so a client validating against `tools/list` reaches the same verdict as the server.

**One entry per tool, so the advertised contract and the executed call are the same thing.** `src/mcp/tools/registry.rs` names each tool exactly once, in an entry that binds it to its argument type, its handler, and the metadata `tools/list` advertises; `src/mcp/tools/args/` declares those argument types with `tool_args!`, and that single field list generates the `inputSchema`, the deserializer, and the validator's allowlist. Two classes of drift become compile errors rather than review items. A tool cannot advertise another tool's schema, because the entry that supplies the schema is the entry that dispatches — mismatching them fails to type-check against the handler's parameter. And a property cannot be advertised without a field behind it, since the schema is generated from the fields; a `tools/list` property no struct accepts fails the round-trip test in `args/`. A third case is guarded rather than proven: a field no handler touches is a `field is never read` build failure under CI's `-D warnings`, which catches the argument wired up and then forgotten, but the lint cannot distinguish reading a field from acting on it, and deliberate non-use is legitimate (`query_logs` ignores the structured filters when `logql` is set). A field's written type carries the rest — `Option<T>` is optional and anything else lands in `required`, `u64` advertises its full range, and a `str_enum!` type advertises its own variants, so a handler matches on an enum exhaustively instead of string-comparing and falling through to a default. Schema validation still runs ahead of deserialization purely for the message: serde reports `invalid type: integer 123, expected a string` without naming the field it was reading, and a message that does not name the argument cannot be acted on. Semantic checks stay in the handlers, where the knowledge lives — a well-typed `min_duration` can still be nonsense, `service` is only required by `reset`'s `service` scope, and only the store knows which services exist.
...
**Architecture.** `src/mcp/` holds `mod.rs` (router), `protocol.rs` (JSON-RPC envelope + version constants), `server.rs` (POST dispatch + transport compliance), `tools/` (`args/` typed arguments plus the schema/validator machinery they generate, `registry.rs` for the one-entry-per-tool table behind both `tools/list` and `tools/call` routing, `dispatch.rs` for request-shape handling and typed argument parsing, and one `handlers/` module per signal), and `synth/` (transport-free typed cores: `summarize_activity`, `check_health`, `describe_service`, `trace_item`, `build_trace_tree`). Handlers stay thin, and they no longer parse: dispatch hands each one its typed arguments, so a handler calls a core/evaluator and formats the response.
